...
// run forever : thread with touch event and min distance value
void runToBall(float speed, int minstop, uint8_t *sensors, uint8_t *motors)
{
    ...
    //thread
    pthread_t tid;
    pthread_create(&tid, NULL, threadRunStraight, (void *)&toThread); // revoir parametres
...
{
    // first: move either left or right with degree amount to be figured out
    int max_speed;
    set_tacho_ramp_up_sp( motors[1], 100);
    set_tacho_ramp_down_sp( motors[1], 100);    
    set_tacho_ramp_up_sp( motors[2], 100);
...
// rotate to compass position: thread with compass and sonar stop when compass value surpassed or touch event